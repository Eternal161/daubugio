...
# CONFIG Bù  Giờ TV
# =========================================================
TARGET_SITE   = "https://sv2.vongcam6.live/lich-thi-dau"
BASE_URL      = "https://sv2.vongcam6.live"
FILE_PATH     = "bugio.json"
LIMIT_MATCHES = 10

VN_TZ = datetime.timezone(datetime.timedelta(hours=7))
GITHUB_TOKEN = os.getenv("GH_TOKEN")
REPO_NAME    = os.getenv("GH_REPO", "Eternal161/daubugio")
...
# =========================================================
# JS: EXTRACT DATA (ĐÃ FIX LẤY THỜI GIAN CHUẨN 100%)
# =========================================================
JS_EXTRACT = """
() => {
    const results = [];
    ...
    for (const a of anchors) {
        ...
        let league = '';
        let timeStr = ''; // 💡 Khởi tạo biến thời gian
        
        const leagueEl = a.querySelector('span.flex-1.truncate');
        if (leagueEl) {
            league = clean(leagueEl.innerText);
            
            // 💡 TUYỆT CHIÊU: Lấy thẻ nằm NGAY CẠNH tên giải (chính là khung chứa thời gian)
            const timeEl = leagueEl.nextElementSibling;
            if (timeEl) {
                // Lấy chữ bên trong và thay thế dấu xuống dòng (nếu có) bằng dấu cách
                timeStr = clean(timeEl.innerText).replace(/\\n/g, ' '); 
            }
        }
...
def capture_stream(context, match_url: str) -> list:
    ...
    for s in streams:
        ...
        if "bglive" in s.lower(): score += 6000
...
def scrape_and_push():
    ...
    print(f"🚀 BẮT ĐẦU BOT Bù  Giờ (Giờ VN): {now_str}")
    ...
    with sync_playwright() as p:
        ...
        try:
            print(f"📺 Đang mở trang Bù  Giờ: {TARGET_SITE}")
        ...
        for m in raw_matches:
            ...
            # Dọn dẹp thêm bằng Python cho chắc chắn
            blv_name = re.sub(r'(?i)đặt cược', '', m.get('blvName', 'BLV Mặc định')).strip()
    ...
    if GITHUB_TOKEN:
        ...
        content = json.dumps({
            "id": "bugio", "name": "Bù  Giờ TV", "last_updated": now_str, 
            "groups": [{"id": "live", "name": "🔴 Live bóng đá Bù  Giờ", "channels": channels}]
        }, indent=2, ensure_ascii=False)
        
        msg = f"⚽ Update Bù  Giờ (VN Time): {now_str}"
        try:
            existing = repo.get_contents(FILE_PATH)
            repo.update_file(existing.path, msg, content, existing.sha)
        except:
            repo.create_file(FILE_PATH, msg, content)
        print("\n✅ HOÀN TẤT CẬP NHẬT Bù  Giờ LÊN GITHUB!")
    else:
